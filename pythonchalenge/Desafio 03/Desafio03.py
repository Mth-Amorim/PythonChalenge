...
for indice in range(len(texto)):
    inicio = indice
    fim = indice + 7 
    conjunto_letas = texto[inicio:fim]
    if conjunto_letas[3].islower():
        if conjunto_letas[:3].isupper() and conjunto_letas[-3:].isupper():
            print(conjunto_letas)


# Tentativa 03
for indice in range(len(texto)):
    novo_texto = texto[indice:indice+9]
    novo_texto = novo_texto.replace('\n',"")
    if len(novo_texto)>=9:
        if novo_texto[0].islower() and novo_texto[-1].islower() and novo_texto[4].islower():
            if novo_texto[1:4].isupper() and novo_texto[-4:-1].isupper():
                print(f"{novo_texto[4]}")

# Resolução do exercico 
import re
resposta = "".join(re.findall('[a-z][A-Z]{3}([a-z])[A-Z]{3}[a-z]', texto))
print(resposta)